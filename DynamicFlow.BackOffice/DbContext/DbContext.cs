...
namespace DynamicFlow.BackOffice.DbContext
{
    public class DbContext : IDbContext
    {
        ...
        public DbContext(IOptions<DatabaseConnection> connection)
        {
        ...
        public async Task<List<T>> GetListQueryAsync<T>(string sp, object? parms = null, string connectionString = "", CommandType commandType = CommandType.Text)
        {
            using var _db = await CreateConnectionAsync(!string.IsNullOrEmpty(connectionString) ? connectionString : _connectionString);
            var Data = await _db.QueryAsync<T>(sp, parms, commandType: commandType);
            return Data.ToList();
